...
#nullable enable
    ...
    public class BubbleDialogueView : DialoguePresenterBase
    {
        ...
        private CancellationTokenSource? contentPresentationCancellationSource;

        [SerializeField] private ActionMarkupHandler[]? eventHandlers;

        private class ForwardingTypewriter : IAsyncTypewriter
        {
            public YarnTask RunTypewriter(MarkupParseResult line, CancellationToken cancellationToken) { return YarnTask.CompletedTask; }
            public void PrepareForContent(MarkupParseResult line) { return; }
            public void ContentWillDismiss() { return; }
            private List<IActionMarkupHandler> markup = new();

            public List<IActionMarkupHandler> ActionMarkupHandlers
            {
                get
                {
                    return markup;
        ...
        public override IAsyncTypewriter? Typewriter
        {
            get
            {
                // if we have an active bubble it will have a typewriter so we can return that
                // if not we return the placeholder
                if (CurrentContentType != ContentType.None)
                {
                    if (currentBubble != null)
                    {
                        if (currentBubble.contentView != null)
                        {
                            if (currentBubble.contentView.typewriter != null)
                            {
                                return currentBubble.contentView.typewriter;
                            }
                        }
                    }
                }
                return placeholderTypewriter;
            }
            set
            {
                Debug.LogError($"{nameof(BubbleDialogueView)} doesn't support setting the typewriter directly. Please change the typewriter on each bubble and their prefab.");
            }
        }
        private ForwardingTypewriter placeholderTypewriter = new();
        ...
        void Start()
        {
            ...
            {
                placeholderTypewriter.ActionMarkupHandlers.AddRange(eventHandlers);
            }
        ...
        private async YarnTask PresentContentInBubble(Bubble bubbleForThisLine, Bubble.BubbleContent content, CancellationToken token)
        {
            ...
            // if we haven't cleaned up the previous presentation cancellation source we will need to do so before making another
            contentPresentationCancellationSource?.Dispose();
            contentPresentationCancellationSource = CancellationTokenSource.CreateLinkedTokenSource(token);

            if (bubbleForThisLine == currentBubble)
            {
                await bubbleForThisLine.UpdateBubble(content, bubbleAdjustSizeDuration, contentPresentationCancellationSource.Token);
            }
            else
            {
                ...
                if (currentBubble != null)
                {
                    await currentBubble.DismissBubble(bubbleHideDuration, contentPresentationCancellationSource.Token);
                }
                currentBubble = bubbleForThisLine;
                await bubbleForThisLine.PresentBubble(content, bubbleShowDuration, contentPresentationCancellationSource.Token);
            }
        ...
        {
            this.currentContent = new Bubble.BubbleContent(line, PlayerName);
            ...
            if (autoAdvance)
            {
                ...
                await YarnTask.Delay(time, token.NextContentToken).SuppressCancellationThrow();
            }
            else
            {
        ...
        public override async YarnTask<DialogueOption?> RunOptionsAsync(DialogueOption[] dialogueOptions, LineCancellationToken cancellationToken)
        {
            ...
            cancellationToken.NextContentToken.Register(() =>
            {
                optionSelectedCompletionSource?.TrySetCanceled();
            });

            // show the current option
            await PresentContentInBubble(bubbleForTheseOptions, this.currentContent, cancellationToken.HurryUpToken);
        ...
        public void CancelBubblePresentation()
        {
            contentPresentationCancellationSource?.Cancel();
        }
        ...
        private Bubble? GetOrCreateBubble(Bubble.BubbleContent content)
        {
            ...
            if (!bubbles.TryGetValue(character, out bubble))
            {
                ...
                // now that we have our anchor we can make the bubble itself
                var freshBubble = CreateBubbleForAnchor(bubbleAnchor);
                if (freshBubble == null)
                {
                    Debug.LogError($"Can't create a bubble for character {character}, was unable to finalise instantiating the prefab.", this);
                    return null;
                }
                bubble = freshBubble;
            }

            // giving the typewriter inside this bubble the action markup
            if (bubble.contentView != null && bubble.contentView.typewriter != null && placeholderTypewriter.ActionMarkupHandlers.Count > 0)
            {
                foreach(var markup in placeholderTypewriter.ActionMarkupHandlers)
                {
                    if (bubble.contentView.typewriter.ActionMarkupHandlers.Contains(markup))
                    {
                        continue;
                    }
                    bubble.contentView.typewriter.ActionMarkupHandlers.Add(markup);
                }
            }
