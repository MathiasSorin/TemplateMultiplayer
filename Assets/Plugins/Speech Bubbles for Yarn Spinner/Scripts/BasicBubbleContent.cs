...
#nullable enable
    ...
    public sealed class BasicBubbleContent : BubbleContentView
    {
        ...
        [SerializeField] int typewriterWordsPerSecond = 5;

        [ShowIf(nameof(typewriterStyle), TypewriterType.Custom)]
        [SerializeField] private InterfaceContainer<IAsyncTypewriter>? Typewriter;
        ...
        void Awake()
        {
            // configuring a typewriter based on our settings
            switch (typewriterStyle)
            {
                ...
                case TypewriterType.ByLetter:
                    ...
                    {
                        CharactersPerSecond = typewriterLettersPerSecond,
                        ActionMarkupHandlers = new List<IActionMarkupHandler>(),
                        Text = textField,
                    };
                    break;
                case TypewriterType.ByWord:
                    ...
                    {
                        WordsPerSecond = typewriterWordsPerSecond,
                        ActionMarkupHandlers = new List<IActionMarkupHandler>(),
                        Text = textField,
                    };
                    break;
                case TypewriterType.Custom:
                    typewriter = Typewriter?.Interface;
                    break;
            }
        }
        ...
        public override async YarnTask PresentContent(Bubble.BubbleContent content, CancellationToken cancellationToken)
        {
            ...
            if (typewriter != null)
            {
                await typewriter.RunTypewriter(content.CurrentElement.Line, cancellationToken).SuppressCancellationThrow();
            }
        }

        public override YarnTask DismissContent(Bubble.BubbleContent content, System.Threading.CancellationToken cancellationToken)
        {
        ...
        public override void PrepareForContent(Bubble.BubbleContent content)
        {
            ...
            if (this.textField != null)
            {
                ...
                if (typewriter != null)
                {
                    typewriter.PrepareForContent(content.CurrentElement.Line);
                }
            }
        }
    }
    ...
    internal static class TMPExtensions
    {
        ...
        public static Vector2 GetRenderedValues(this TMP_Text textMeshPro, string text, float maxWidth = float.MaxValue, float maxHeight = float.MaxValue, bool onlyVisibleCharacters = true)
        {
            ...
            if (IsInvalidFloat(renderedSize.x) || IsInvalidFloat(renderedSize.y) || renderedSize.x > maxWidth || renderedSize.y > maxHeight)
            {
                ...
                preferredSize = new Vector2(Mathf.Clamp(preferredSize.x, 0, maxWidth), Mathf.Clamp(preferredSize.y, 0, maxHeight));
                renderedSize = preferredSize;
            }
